...
class ModelData:
    def __init__(
        ...
        if self.process_method == MDX_ARCH_TYPE:
            ...
            self.chunks = (
                root.determine_auto_chunks(
                    root.chunks_var.get(), self.is_gpu_conversion
                )
                if root.is_chunk_mdxnet_var.get()
                else 0
            )
        ...
        if self.process_method == DEMUCS_ARCH_TYPE:
            ...
            self.chunks_demucs = root.determine_auto_chunks(
                root.chunks_demucs_var.get(), self.is_gpu_conversion
            )
        ...
        if self.is_secondary_model_activated and self.model_status:
            if (
                ...
                and root.demucs_stems_var.get() == ALL_STEMS
                ...
                self.is_secondary_model_activated = (
                    False
                    if all(i is None for i in self.secondary_model_4_stem)
                    else True
                )
                self.demucs_4_stem_added_count = (
                    self.demucs_4_stem_added_count - 1
                    if self.is_secondary_model_activated
                    else self.demucs_4_stem_added_count
                )
                if self.is_secondary_model_activated:
                    self.secondary_model_4_stem_model_names_list = [
                        None if i is None else i.model_basename
                ...
                primary_stem = (
                    ...
                    if self.is_ensemble_mode and self.process_method == DEMUCS_ARCH_TYPE
    ...
    def get_demucs_model_data(self):
        ...
        self.demucs_source_list = (
            DEMUCS_2_SOURCE if DEMUCS_UVR_MODEL in self.model_name else DEMUCS_4_SOURCE
        )
        self.demucs_source_map = (
            DEMUCS_2_SOURCE_MAPPER
            if DEMUCS_UVR_MODEL in self.model_name
            else DEMUCS_4_SOURCE_MAPPER
        )
    ...
    def get_model_data(self, model_hash_dir, hash_mapper):
        model_settings_json = os.path.join(
            model_hash_dir, "{}.json".format(self.model_hash)
        )
...
class Ensembler:
    def __init__(self, is_manual_ensemble=False):
        ...
        chosen_ensemble_name = (
            "{}".format(root.chosen_ensemble_var.get().replace(" ", "_"))
            if not root.chosen_ensemble_var.get() == CHOOSE_ENSEMBLE_OPTION
            else "Ensembled"
        )
        ...
        self.chosen_ensemble = (
            f"_{chosen_ensemble_name}" if root.is_append_ensemble_name_var.get() else ""
        )
        ensemble_folder_name = (
            self.main_export_path
            if self.is_save_all_outputs_ensemble
            else ENSEMBLE_TEMP_PATH
        )
        self.ensemble_folder_name = os.path.join(
            ensemble_folder_name,
            "{}_Outputs_{}".format(chosen_ensemble_name, time_stamp),
        )
    ...
    def ensemble_outputs(
        self, audio_file_base, export_path, stem, is_4_stem=False, is_inst_mix=False
    ):
    ...
    def ensemble_manual(self, audio_inputs, audio_file_base, is_bulk=False):
        ...
        if is_bulk:
            number_list = list(
                set([os.path.basename(i).split("_")[0] for i in audio_inputs])
            )
            for n in number_list:
                current_list = [
                    i for i in audio_inputs if os.path.basename(i).startswith(n)
                ]
                audio_file_base = os.path.basename(current_list[0]).split(".wav")[0]
                stem_testing = (
                    "instrum" if "Instrumental" in audio_file_base else "vocals"
                )
                if is_mv_sep:
                    audio_file_base = audio_file_base.split("_")
                    audio_file_base = (
                        f"{audio_file_base[1]}_{audio_file_base[2]}_{stem_testing}"
                    )
    ...
    def ensemble_manual_process(self, audio_inputs, audio_file_base, is_bulk):
        ...
        stem_save_path = os.path.join(
            ...
            "{}{}{}.wav".format(self.is_testing_audio, audio_file_base, algorithm_text),
        )
        spec_utils.ensemble_inputs(
            audio_inputs,
            algorithm,
...
class UVRInterface:
    ...
    def assemble_model_data(
        self, model=None, arch_type=ENSEMBLE_MODE, is_dry_check=False
    ) -> List[ModelData]:
        if arch_type == ENSEMBLE_STEM_CHECK:
            model_data = self.model_data_table
            missing_models = [
                model.model_status for model in model_data if not model.model_status
            ]

            if missing_models or not model_data:
                model_data: List[ModelData] = [
                    ModelData(model_name, is_dry_check=is_dry_check)
                    for model_name in self.ensemble_model_list
                ]
        ...
        if arch_type == ENSEMBLE_MODE:
            model_data: List[ModelData] = [
                ...
                for model_name in self.ensemble_listbox_get_all_selected_models()
            ]
    ...
    def create_sample(self, audio_file, sample_path=SAMPLE_CLIP_PATH):
        ...
        sample = librosa.load(
            audio_file, offset=offset_cut, duration=clip_duration, mono=False, sr=44100
        )[0].T
        audio_sample = os.path.join(
            sample_path,
            f"{os.path.splitext(os.path.basename(audio_file))[0]}_{name_apped}sample.wav",
        )
    ...
    def verify_audio(self, audio_file, is_process=True, sample_path=None):
        ...
        if os.path.isfile(audio_file):
            try:
                librosa.load(audio_file, duration=3, mono=False, sr=44100) if not type(
                    sample_path
                ) is str else self.create_sample(audio_file, sample_path)
                is_good = True
            except Exception as e:
                ...
                if is_process:
                    ...
                    self.error_log_var.set(
                        f'Error Loading the Following File:\n\n"{audio_base_name}"\n\nRaw Error Details:\n\n{message}'
                    )
                else:
    ...
    def cached_source_model_list_check(self, model_list: List[ModelData]):
        model: ModelData
        primary_model_names = lambda process_method: [
            model.model_basename if model.process_method == process_method else None
            for model in model_list
        ]
        secondary_model_names = lambda process_method: [
            model.secondary_model.model_basename
            if model.is_secondary_model_activated
            and model.process_method == process_method
            else None
            for model in model_list
        ]
        ...
        self.demucs_secondary_model_names = [
            model.secondary_model.model_basename
            if model.is_secondary_model_activated
            and model.process_method == DEMUCS_ARCH_TYPE
            and not model.secondary_model is None
            else None
            for model in model_list
        ]
        self.demucs_pre_proc_model_name = [
            model.pre_proc_model.model_basename if model.pre_proc_model else None
            for model in model_list
        ]  # list(dict.fromkeys())

        for model in model_list:
            if (
                model.process_method == DEMUCS_ARCH_TYPE
                and model.is_demucs_4_stem_secondaries
            ):
    ...
    def process(
        ...
        time_elapsed = (
            lambda: f'Time Elapsed: {time.strftime("%H:%M:%S", time.gmtime(int(time.perf_counter() - stime)))}'
        )
        ...
        if self.verify_audio(audio_file):
            audio_file = (
                self.create_sample(audio_file) if is_model_sample_mode else audio_file
            )
        else:
        ...
        audio_file_base = (
            audio_file_base
            if is_ensemble
            else f"{round(time.time())}_{audio_file_base}"
        )
        audio_file_base = (
            audio_file_base
            if not is_ensemble
            else f"{audio_file_base}_{model.model_basename}"
        )
        if not is_ensemble:
        ...
        process_data = {
            ...
            "write_to_console": lambda progress_text, base_text="": console_write(
        ...
        if is_ensemble:
            ...
            if is_4_stem_ensemble:
                ...
                if not root.is_secondary_stem_only_var.get():
                    ensemble.ensemble_outputs(
                        audio_file_base, export_path, PRIMARY_STEM
                    )
                if not root.is_primary_stem_only_var.get():
                    ensemble.ensemble_outputs(
                        audio_file_base, export_path, SECONDARY_STEM
                    )
                    ensemble.ensemble_outputs(
        ...
        console_write(
            f"Process Complete, using time: {time_elapsed()}\nOutput path: {export_path}"
        )
...
class FakeRoot:
    ...
    def load_saved_settings(self, loaded_setting: dict, process_method=None):
        ...
        if not process_method or process_method == VR_ARCH_PM or is_ensemble:
            ...
            self.post_process_threshold_var.set(
                loaded_setting["post_process_threshold"]
            )
            self.vr_voc_inst_secondary_model_var.set(
                loaded_setting["vr_voc_inst_secondary_model"]
            )
            self.vr_other_secondary_model_var.set(
                loaded_setting["vr_other_secondary_model"]
            )
            self.vr_bass_secondary_model_var.set(
                loaded_setting["vr_bass_secondary_model"]
            )
            self.vr_drums_secondary_model_var.set(
                loaded_setting["vr_drums_secondary_model"]
            )
            self.vr_is_secondary_model_activate_var.set(
                loaded_setting["vr_is_secondary_model_activate"]
            )
            self.vr_voc_inst_secondary_model_scale_var.set(
                loaded_setting["vr_voc_inst_secondary_model_scale"]
            )
            self.vr_other_secondary_model_scale_var.set(
                loaded_setting["vr_other_secondary_model_scale"]
            )
            self.vr_bass_secondary_model_scale_var.set(
                loaded_setting["vr_bass_secondary_model_scale"]
            )
            self.vr_drums_secondary_model_scale_var.set(
        ...
        if not process_method or process_method == DEMUCS_ARCH_TYPE or is_ensemble:
            ...
            self.is_primary_stem_only_Demucs_var.set(
                loaded_setting["is_primary_stem_only_Demucs"]
            )
            self.is_secondary_stem_only_Demucs_var.set(
                loaded_setting["is_secondary_stem_only_Demucs"]
            )
            self.is_split_mode_var.set(loaded_setting["is_split_mode"])
            self.is_demucs_combine_stems_var.set(
                loaded_setting["is_demucs_combine_stems"]
            )
            self.demucs_voc_inst_secondary_model_var.set(
                loaded_setting["demucs_voc_inst_secondary_model"]
            )
            self.demucs_other_secondary_model_var.set(
                loaded_setting["demucs_other_secondary_model"]
            )
            self.demucs_bass_secondary_model_var.set(
                loaded_setting["demucs_bass_secondary_model"]
            )
            self.demucs_drums_secondary_model_var.set(
                loaded_setting["demucs_drums_secondary_model"]
            )
            self.demucs_is_secondary_model_activate_var.set(
                loaded_setting["demucs_is_secondary_model_activate"]
            )
            self.demucs_voc_inst_secondary_model_scale_var.set(
                loaded_setting["demucs_voc_inst_secondary_model_scale"]
            )
            self.demucs_other_secondary_model_scale_var.set(
                loaded_setting["demucs_other_secondary_model_scale"]
            )
            self.demucs_bass_secondary_model_scale_var.set(
                loaded_setting["demucs_bass_secondary_model_scale"]
            )
            self.demucs_drums_secondary_model_scale_var.set(
            ...
            self.is_demucs_pre_proc_model_activate_var.set(
                data["is_demucs_pre_proc_model_activate"]
            )
            self.is_demucs_pre_proc_model_inst_mix_var.set(
                data["is_demucs_pre_proc_model_inst_mix"]
            )
        ...
        if not process_method or process_method == MDX_ARCH_TYPE or is_ensemble:
            ...
            self.mdx_voc_inst_secondary_model_var.set(
                loaded_setting["mdx_voc_inst_secondary_model"]
            )
            self.mdx_other_secondary_model_var.set(
                loaded_setting["mdx_other_secondary_model"]
            )
            self.mdx_bass_secondary_model_var.set(
                loaded_setting["mdx_bass_secondary_model"]
            )
            self.mdx_drums_secondary_model_var.set(
                loaded_setting["mdx_drums_secondary_model"]
            )
            self.mdx_is_secondary_model_activate_var.set(
                loaded_setting["mdx_is_secondary_model_activate"]
            )
            self.mdx_voc_inst_secondary_model_scale_var.set(
                loaded_setting["mdx_voc_inst_secondary_model_scale"]
            )
            self.mdx_other_secondary_model_scale_var.set(
                loaded_setting["mdx_other_secondary_model_scale"]
            )
            self.mdx_bass_secondary_model_scale_var.set(
                loaded_setting["mdx_bass_secondary_model_scale"]
            )
            self.mdx_drums_secondary_model_scale_var.set(
                loaded_setting["mdx_drums_secondary_model_scale"]
            )

        if not process_method or is_ensemble:
            self.is_save_all_outputs_ensemble_var.set(
                loaded_setting["is_save_all_outputs_ensemble"]
            )
            self.is_append_ensemble_name_var.set(
                loaded_setting["is_append_ensemble_name"]
            )
            ...
            self.is_secondary_stem_only_var.set(
                loaded_setting["is_secondary_stem_only"]
            )
            ...
            self.is_create_model_folder_var.set(
                loaded_setting["is_create_model_folder"]
            )
        ...
        self.model_sample_mode_duration_var.set(
            loaded_setting["model_sample_mode_duration"]
        )
