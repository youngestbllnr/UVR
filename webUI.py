...
class UVRWebUI:
    ...
    def process(
        self, input_audios, model_name, arch, setting1, setting2, progress=gr.Progress()
    ):
        results = []
        for input_audio in input_audios:
            ...
            sampling_rate, audio = input_audio[0]
    ...
    def define_layout(self):
        with gr.Blocks() as app:
                ...
                with gr.TabItem("Process"):
                    with gr.Row():
                        self.arch_choice = gr.Dropdown(
                            ...
                            value=VR_ARCH_TYPE,
                            label=CHOOSE_PROC_METHOD_MAIN_LABEL,
                            interactive=True,
                        )
                    ...
                    with gr.Row():
                        self.use_gpu = gr.Checkbox(
                            label=GPU_CONVERSION_MAIN_LABEL,
                            value=root.is_gpu_conversion_var.get(),
                            interactive=True,
                        )
                        self.primary_stem_only = gr.Checkbox(
                            label=f"{PRIMARY_STEM} only",
                            value=root.is_primary_stem_only_var.get(),
                            interactive=True,
                        )
                        self.secondary_stem_only = gr.Checkbox(
                            label=f"{SECONDARY_STEM} only",
                            value=root.is_secondary_stem_only_var.get(),
                            interactive=True,
                        )
                        self.sample_mode = gr.Checkbox(
                            label=SAMPLE_MODE_CHECKBOX(
                                root.model_sample_mode_duration_var.get()
                            ),
                            value=root.model_sample_mode_var.get(),
                            interactive=True,
                        )
                ...
                with gr.TabItem("Settings"):
                    with gr.Tabs():
                        with gr.TabItem("Additional Settings"):
                            self.wav_type = gr.Dropdown(
                                choices=WAV_TYPE,
                                label="Wav Type",
                                value="PCM_16",
                                interactive=True,
                            )
                            self.mp3_rate = gr.Dropdown(
                                choices=MP3_BIT_RATES,
                                label="MP3 Bitrate",
                                value="320k",
                                interactive=True,
                            )
            ...
            self.arch_choice.change(
                self.arch_select_update,
                inputs=self.arch_choice,
            ...
            self.set_checkboxes_exclusive(
                ...
                [
                    lambda value: root.is_primary_stem_only_var.set(value),
                    lambda value: root.is_secondary_stem_only_var.set(value),
                ],
            )
